...
// 2. Validate Tour ID
$visite_id = intval($_GET['visite_id'] ?? 0);
if ($visite_id <= 0) { header('Location: /ASSAD/Guide/Tours/tours.php'); exit(); }

// 3. Get Tour Details & Verify Ownership
$stmt = $conn->prepare("SELECT titre, id_user FROM visitesguidees WHERE id_visite = ?");
$stmt->bind_param("i", $visite_id);
$stmt->execute();
$tour = $stmt->get_result()->fetch_assoc();

// If tour doesn't exist or doesn't belong to this guide
if (!$tour || $tour['id_user'] != $guideId) {
    header('Location: /ASSAD/Guide/Tours/tours.php?error=forbidden');
    exit();
}

// 4. Fetch Steps
$stmt = $conn->prepare("SELECT * FROM etapesvisite WHERE id_visite = ? ORDER BY ordreetape ASC");
$stmt->bind_param("i", $visite_id);
$stmt->execute();
...
while ($row = $result->fetch_assoc()) {
    $steps[] = $row;
    $totalDuration += $row['duree_minutes']; // Summing up duration
}
?>
