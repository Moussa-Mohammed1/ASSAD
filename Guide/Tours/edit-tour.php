...
if (intval($tour['id_user']) !== $guideId) {
    header('Location: /ASSAD/Guide/Tours/tours.php?error=forbidden');
    exit();
}

$titre = $tour['titre'] ?? '';
$date = $tour['dateheure'] ? date('Y-m-d', strtotime($tour['dateheure'])) : '';
$time = $tour['dateheure'] ? date('H:i', strtotime($tour['dateheure'])) : '';
$langue = $tour['langue'] ?? '';
$duree = $tour['duree'] ?? '';
$prix = $tour['prix'] ?? '';
$capacite_max = $tour['capacite_max'] ?? '';
            ...
            <div class="flex items-center gap-4 sm:gap-8">
                <nav class="hidden md:flex items-center gap-6">
                    <a class="text-gray-600 dark:text-gray-300 hover:text-primary dark:hover:text-primary text-sm font-medium transition-colors"
                        href="/ASSAD/Guide/dashboard.php">Home</a>
    ...
    <main class="flex-grow flex justify-center py-6 sm:py-10 px-4 sm:px-6 md:px-10 lg:px-40">
        <div class="flex flex-col w-full max-w-4xl gap-6">
            <div class="flex flex-wrap gap-2 text-sm">
                ...
                <a class="text-gray-500 dark:text-[#9db9a6] hover:underline" href="#">My Guided Tours</a>
                <span class="text-gray-400 dark:text-[#5e7164]">/</span>
            ...
            <div
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <p class="text-gray-500 dark:text-[#9db9a6] text-base max-w-2xl">
                        Modify details for <span class="text-gray-900 dark:text-white font-semibold">"Lions of the
                            Serengeti: Sunset Prowl"</span>.
                    </p>
                </div>
                <div
            ...
            <div
                ...
                <div class="p-6 md:p-8">
                    <form action="/ASSAD/Guide/Tours/Actions/edit.php" method="POST" class="flex flex-col gap-8">
                        <input type="hidden" name="id_visite" value="<?= $id ?>" />
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="col-span-1 md:col-span-2">
                                ...
                                <input name="titre"
                                    ...
                                    type="text" value="<?= htmlspecialchars($titre) ?>" />
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <select name="langue"
                                        ...
                                        <option <?= $langue === 'English' ? 'selected' : '' ?>>English</option>
                                        <option <?= $langue === 'French' ? 'selected' : '' ?>>French</option>
                                        <option <?= $langue === 'Portuguese' ? 'selected' : '' ?>>Portuguese</option>
                                        <option <?= $langue === 'Spanish' ? 'selected' : '' ?>>Spanish</option>
                                    </select>
                                    <span
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                                <div>
                                    ...
                                    <input name="date"
                                        ...
                                        type="date" value="<?= htmlspecialchars($date) ?>" />
                                </div>
                                <div>
                                    <label
                                        class="block text-xs font-semibold text-gray-500 dark:text-[#9db9a6] mb-1.5 uppercase">Start
                                        Time</label>
                                    <input name="time"
                                        class="w-full h-10 px-3 rounded-lg bg-white dark:bg-[#1a2920] border-gray-200 dark:border-[#28392e] text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent text-sm [color-scheme:light] dark:[color-scheme:dark]"
                                        type="time" value="<?= htmlspecialchars($time) ?>" />
                                </div>
                                <div>
                                    <label
                                    ...
                                    <input name="duree"
                                        ...
                                        type="number" value="<?= htmlspecialchars($duree) ?>" />
                                </div>
                                <div>
                                    <label
                                    ...
                                    <div class="relative">
                                        ...
                                        <input name="prix"
                                            ...
                                            step="0.01" type="number" value="15.00" />
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-end gap-4 justify-between">
                                    <div class="w-full sm:w-1/3">
                                        ...
                                        <input name="capacite_max"
                                            class="w-full h-10 px-3 rounded-lg bg-white dark:bg-[#1a2920] border-gray-200 dark:border-[#28392e] text-gray-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent text-sm"
                                            type="number" value="<?= htmlspecialchars($capacite_max) ?>" />
                                    </div>
                                    <div class="flex-1">
                                        <div
                                            class="flex justify-between text-xs font-semibold uppercase tracking-wider text-gray-500 dark:text-[#9db9a6] mb-2">
                                            <span>Current Bookings: <?= intval($tour['booked_count'] ?? 0) ?>
                                                Visitors</span>
                                            <span
                                                class="text-primary"><?= $capacite_max ? intval((($tour['booked_count'] ?? 0) / $capacite_max) * 100) : 0 ?>%
                                                Full</span>
                                        </div>
                                        <div
                                            ...
                                            <div class="h-full bg-primary rounded-full" style="width: <?php 
                                                $stmt = $conn->prepare('SELECT COUNT(*) AS total_reservations FROM reservation WHERE id_visite = ?');
                                                $stmt->bind_param('i', $tour['id_visite']);
                                                $stmt->execute();
                                        ...
                                        <p class="text-xs text-orange-500 mt-2 flex items-center gap-1">
                                            ...
                                            Cannot decrease capacity below current bookings (<?=$total['total_reservations']?>).
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div
                            ...
                            <a 
                                href="./actions/cancel.php?tour=<?=$tour['id_visite']?>"
                            ...
                            <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                                <button
                                    class="w-full sm:w-auto px-6 h-11 rounded-lg border border-gray-300 dark:border-[#28392e] text-gray-700 dark:text-gray-200 font-bold text-sm hover:bg-gray-50 dark:hover:bg-[#28392e] transition-colors"
